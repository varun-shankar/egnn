...
# Callbacks
class RPB(RichProgressBar):
    def get_metrics(self, trainer, model):
        items = super().get_metrics(trainer, model)
        items.pop('v_num', None)
        its = items.copy()
        for k,v in items.items():
            its[k] = f'{v:.3e}' if isinstance(v, float) else v
        return its
rpb = RPB(), RichModelSummary(max_depth=3)
...
timer = Timer(duration="00:06:00:00")
...
if config.get('job_type') != 'eval':
    ...
    trainer = pl.Trainer(accelerator='gpu',
        devices=config.get('gpus'), strategy=strategy, precision='16-mixed',
